...
struct Mpz : public mpz_class
{
	Mpz(pybind11::int_ i);
	Mpz(const mpz_class& i);
	operator pybind11::int_();
};
...
public:
	...
	DiffieHellman(Mpz p, Mpz g, Mpz ya);
...
public:
	...
	RSA(Mpz e, Mpz d, Mpz K);//read frem certificate or memory
	pybind11::int_ decode(pybind11::int_ m), encode(pybind11::int_ m);
	mpz_class K, e;
protected:
	mpz_class p, q, d, phi;
};
